...
(async function getData() {
  ...
  const searchQuery = 'searchQueryState={'+
      '"pagination":{},'+
      '"usersSearchTerm":"Vancouver, BC",'+
      '"mapBounds":{'+
        '"west":-124.39390043012746,'+
        '"east":-121.85419706098683,'+
        '"south":48.96089395186731,'+
        '"north":49.579742125071834},'+
    '"mapZoom":9,'+
    '"isMapVisible":true,'+
    '"filterState":{'+
    ...
    '"isListVisible":true}'+
    '&wants={"cat1":["listResults","mapResults"]}&requestId=6'
  ...
  // console.log(data)
  console.log(data.cat1.searchResults.mapResults.length)
  ...
  const geojson = {
    ...
    features: data.cat1.searchResults.mapResults.filter(v=>v.hdpData).map(({hdpData:{homeInfo:i}, ...d})=>({
      ...
      properties: {
        ...
        lotAreaValue: i.lotAreaValue
      }
    }))
  }
  fs.writeFile(`./data/zillow_sold_${(new Date()).toISOString().split('T')[0]}.geojson`, JSON.stringify(geojson), {}, (err, data)=>{if(err){console.log(err)}})
